...
# Script para automatizar la generación del reporte de cobertura con xUnit
...
dotnet test .\KairosPWA.Tests\KairosPWA.Tests.csproj `
  --collect:"XPlat Code Coverage" `
  --results-directory .\KairosPWA.Tests\TestResults
...
# Paso 2: Obtener la carpeta más reciente de TestResults
Write-Host "[2/3] Localizando archivo de cobertura..." -ForegroundColor Yellow
$latestCoverage = Get-ChildItem .\KairosPWA.Tests\TestResults | 
...
if (-not (Test-Path $coverageXml)) {
    Write-Host "ERROR: No se encontró $coverageXml" -ForegroundColor Red
    exit 1
}
...
if (-not (Test-Path $reportGeneratorPath)) {
    Write-Host "ERROR: ReportGenerator no está instalado globalmente" -ForegroundColor Red
    Write-Host "Instálalo con: dotnet tool install -g dotnet-reportgenerator-globaltool" -ForegroundColor Yellow
    exit 1
}

& $reportGeneratorPath `
  -reports:$coverageXml `
  -targetdir:".\KairosPWA.Tests\coverage-report" `
  -reporttypes:Html

if ($LASTEXITCODE -ne 0) {
    Write-Host "ERROR: ReportGenerator falló" -ForegroundColor Red
    exit 1
}
...
if (Test-Path $reportPath) {
    ...
    Write-Host "ADVERTENCIA: No se pudo abrir el reporte automáticamente" -ForegroundColor Yellow
